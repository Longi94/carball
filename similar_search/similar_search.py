sizes_per_layer = [7, 7, 5]
num_layers = 3

class SimilarSearch:

    @staticmethod
    def create_layers_from_point(point) -> tuple:
        output = []
        for i in range(num_layers):
            layer_output = []
            for j in range(len(point)):
                divsor = max(1, sizes_per_layer[j] * i)
                layer_coordinate = (point[j] // divsor) % (sizes_per_layer[j] * (i + 1))
                layer_output.append(layer_coordinate)
            output.append(tuple(layer_output))
        return tuple(output)
    ...
    def create_hashes_for_search(self, ):
